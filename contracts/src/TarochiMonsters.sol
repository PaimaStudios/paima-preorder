...
import {ITarochiMonsters} from "./ITarochiMonsters.sol";

/// @dev A standard ERC721 that accepts calldata in the mint function for any initialization data needed in a Paima dApp.
contract TarochiMonsters is ITarochiMonsters, ERC165, ERC721, Ownable {
    ...
    modifier onlyTokenOwner(uint256 tokenId) {
        require(msg.sender == ownerOf(tokenId), "TarochiMonsters: not owner");
        _;
    }
    ...
    function supportsInterface(bytes4 interfaceId) public view override(ERC165, ERC721) returns (bool) {
        return interfaceId == type(ITarochiMonsters).interfaceId || super.supportsInterface(interfaceId);
    }
    ...
    function mint(address _to, string calldata initialData) external returns (uint256) {
        require(_to != address(0), "TarochiMonsters: zero receiver address");
    ...
    function tokenURI(uint256 tokenId, string memory customBaseUri) public view returns (string memory) {
        _requireOwned(tokenId);
        string memory URI = bytes(customBaseUri).length > 0 ? string.concat(customBaseUri, tokenId.toString()) : "";
